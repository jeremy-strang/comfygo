...
class InjectNoiseHook(PixelKSampleHook):
    ...
    def post_encode(self, samples):
        ...
        if "BNK_NoisyLatentImage" in _nodes.NODE_CLASS_MAPPINGS and "BNK_InjectNoise" in _nodes.NODE_CLASS_MAPPINGS:
            NoisyLatentImage = _nodes.NODE_CLASS_MAPPINGS["BNK_NoisyLatentImage"]
            InjectNoise = _nodes.NODE_CLASS_MAPPINGS["BNK_InjectNoise"]
        else:
...
class LamaRemoverDetailerHook(DetailerHook):
    ...
    def post_upscale(self, img, mask=None):
        import nodes as _nodes
        if "LamaRemover" in _nodes.NODE_CLASS_MAPPINGS:
            lama_remover_obj = _nodes.NODE_CLASS_MAPPINGS['LamaRemover']()
        else:
...
class PreviewDetailerHook(DetailerHook):
    ...
    async def send(self, image):
        ...
        import tempfile
        temp_path = os.path.join(tempfile.gettempdir(), 'pvhook')
