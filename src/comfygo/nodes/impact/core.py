...
def enhance_detail(image, model, clip, vae, guide_size, guide_size_for_bbox, max_size, bbox, seed, steps, cfg,
    ...
    if wildcard_opt is not None and wildcard_opt != "":
        ...
        if wildcard_opt_concat_mode == "concat":
            ...
            positive = _nodes.ConditioningConcat().concat(positive, wildcard_positive)[0]
        else:
    ...
    if detailer_hook is None or not detailer_hook.get_skip_sampling():
        if noise_mask is not None and inpaint_model:
            import nodes as _nodes
            imc_encode = _nodes.InpaintModelConditioning().encode
        ...
        if vae_tiled_decode:
            import nodes as _nodes
            (refined_image,) = _nodes.VAEDecodeTiled().decode(vae, refined_latent, 512) # using default settings
...
def enhance_detail_for_animatediff(image_frames, model, clip, vae, guide_size, guide_size_for_bbox, max_size, bbox, seed, steps, cfg,
    ...
    import nodes as _nodes
    refined_image_frames = _nodes.ImageScale().upscale(image=refined_image_frames, upscale_method='lanczos', width=w, height=h, crop='disabled')[0]
...
class ESAMWrapper:
    def __init__(self, model, device):
        self.model = model
        import nodes as _nodes
        self.func_inference = _nodes.NODE_CLASS_MAPPINGS['Yoloworld_ESAM_Zho']
        self.device = device
...
def vae_encode(vae, pixels, use_tile, hook, tile_size=512, overlap=64):
    if use_tile:
        ...
        samples = {"samples": vae.encode(pixels)}
...
def latent_upscale_on_pixel_space_shape2(samples, scale_method, w, h, vae, use_tile=False, tile_size=512, save_temp_prefix=None, hook=None, overlap=64):
    ...
    if save_temp_prefix is not None:
        import nodes as _nodes
        _nodes.PreviewImage().save_images(pixels, filename_prefix=save_temp_prefix)

    import nodes as _nodes
    pixels = _nodes.ImageScale().upscale(pixels, scale_method, int(w), int(h), False)[0]
...
def latent_upscale_on_pixel_space_with_model_shape2(samples, scale_method, upscale_model, new_w, new_h, vae, use_tile=False, tile_size=512, save_temp_prefix=None, hook=None, overlap=64):
    ...
    while current_w < new_w:
        import nodes as _nodes
        model_upscaler = _nodes.NODE_CLASS_MAPPINGS['ImageUpscaleWithModel']()
    ...
    import nodes as _nodes
    pixels = _nodes.ImageScale().upscale(pixels, scale_method, int(new_w), int(new_h), False)[0]
    ...
    def sample(self, model, seed, steps, cfg, sampler_name, scheduler, positive, negative, upscaled_latent, denoise, images):
        if self.tile_cnet is not None:
            ...
            if image_batch > 1:
                ...
                import nodes as _nodes
                if 'TilePreprocessor' not in _nodes.NODE_CLASS_MAPPINGS:
                    raise RuntimeError("'TilePreprocessor' node (from comfyui_controlnet_aux) isn't installed.")
                preprocessor = _nodes.NODE_CLASS_MAPPINGS['TilePreprocessor']()
                ...
                positive, negative = _nodes.ControlNetApplyAdvanced().apply_controlnet(positive=positive,
                                                                                      negative=negative,
...
class IPAdapterWrapper:
    ...
    def doit_ipadapter(self, model):
        ...
        import nodes as _nodes
        if 'IPAdapterAdvanced' not in _nodes.NODE_CLASS_MAPPINGS:
            if 'IPAdapterApply' in _nodes.NODE_CLASS_MAPPINGS:
        ...
        obj = _nodes.NODE_CLASS_MAPPINGS['IPAdapterAdvanced']
...
class ControlNetWrapper:
    ...
    def apply(self, positive, negative, image, mask=None, use_acn=False):
        ...
        import nodes as _nodes
        if use_acn:
            if "ACN_AdvancedControlNetApply" in _nodes.NODE_CLASS_MAPPINGS:
                acn = _nodes.NODE_CLASS_MAPPINGS['ACN_AdvancedControlNetApply']()
            ...
            positive = _nodes.ControlNetApply().apply_controlnet(positive, self.control_net, cnet_image, self.strength)[0]
    ...
    def apply(self, positive, negative, image, mask=None, use_acn=False):
        ...
        if use_acn:
            ...
            if self.vae is not None:
                apply_controlnet = _nodes.ControlNetApplyAdvanced().apply_controlnet
                signature = inspect.signature(apply_controlnet)

                if 'vae' in signature.parameters:
                    positive, negative = _nodes.ControlNetApplyAdvanced().apply_controlnet(positive, negative, self.control_net, cnet_image, self.strength, self.start_percent, self.end_percent, vae=self.vae)
                else:
                ...
                positive, negative = _nodes.ControlNetApplyAdvanced().apply_controlnet(positive, negative, self.control_net, cnet_image, self.strength, self.start_percent, self.end_percent)
...
class TiledKSamplerWrapper:
    ...
    def sample(self, latent_image, hook=None):
        import nodes as _nodes
        if "BNK_TiledKSampler" in _nodes.NODE_CLASS_MAPPINGS:
            TiledKSampler = _nodes.NODE_CLASS_MAPPINGS['BNK_TiledKSampler']
        else:
            utils.try_install_custom_node('https://github.com/BlenderNeko/ComfyUI_TiledKSampler',
...
class BBoxDetectorBasedOnCLIPSeg:
    ...
    def detect_combined(self, image, bbox_threshold, bbox_dilation):
        import nodes as _nodes
        if "CLIPSeg" in _nodes.NODE_CLASS_MAPPINGS:
            CLIPSeg = _nodes.NODE_CLASS_MAPPINGS['CLIPSeg']
        else:
...
def update_node_status(node, text, progress=None):
    # No-op: PromptServer not available outside ComfyUI
    return
        ...
        if method != LatentPreviewMethod.NoPreviews or force:
            ...
            if hasattr(latent_format, "taesd_decoder_path"):
                try:
                    ...
                    taesd_decoder_path = _fp.get_full_path("vae_approx", latent_format.taesd_decoder_name)
                except Exception:
                    taesd_decoder_path = None
