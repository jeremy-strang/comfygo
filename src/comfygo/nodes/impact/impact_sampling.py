...
def calculate_sigmas(model, sampler, scheduler, steps):
    ...
    if scheduler.startswith('AYS'):
        ...
        sigmas = _nodes.NODE_CLASS_MAPPINGS['AlignYourStepsScheduler']().get_sigmas(scheduler[4:], steps, denoise=1.0)[0]
    elif scheduler.startswith('GITS[coeff='):
        import nodes as _nodes
        sigmas = _nodes.NODE_CLASS_MAPPINGS['GITSScheduler']().execute(float(scheduler[11:-1]), steps, denoise=1.0)[0]
    elif scheduler == 'LTXV[default]':
        import nodes as _nodes
        sigmas = _nodes.NODE_CLASS_MAPPINGS['LTXVScheduler']().execute(20, 2.05, 0.95, True, 0.1)[0]
    elif scheduler.startswith('OSS'):
        import nodes as _nodes
        sigmas = _nodes.NODE_CLASS_MAPPINGS['OptimalStepsScheduler']().execute(scheduler[4:], steps, denoise=1.0)[0]
    else:
...
def ksampler_wrapper(model, seed, steps, cfg, sampler_name, scheduler, positive, negative, latent_image, denoise,
    ...
    if refiner_ratio is None or refiner_model is None or refiner_clip is None or refiner_positive is None or refiner_negative is None:
        ...
        if 'noise_mask' in latent_image:
            import nodes as _nodes
            latent_compositor = _nodes.NODE_CLASS_MAPPINGS['LatentCompositeMasked']()
